...
@section('content')
    ...
    <div class="mobile-content">
        ...
        @forelse($workplaces as $workplace)
            <div class="mobile-card mb-3" onclick="window.location='{{ route('mobile.workplaces.show', $workplace->id) }}'">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                        <h6 class="mb-1">{{ $workplace->name }}</h6>
                    ...
                    <div class="text-end">
                        @if($workplace->current_assignments_count > 0)
                            <span class="mobile-badge mobile-badge-success">{{ $workplace->current_assignments_count }} {{ __('employees') }}</span>
                        @else
                            <span class="mobile-badge mobile-badge-secondary">{{ __('No employees') }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div class="mobile-card-stats">
                    <div class="mobile-stat-item">
                        <span class="mobile-stat-label">{{ __('Employees') }}</span>
                        <span class="mobile-stat-value">{{ $workplace->current_assignments_count }}</span>
                    </div>
                    <div class="mobile-stat-item">
                        <span class="mobile-stat-label">{{ __('Positions') }}</span>
                        <span class="mobile-stat-value">{{ $workplace->positions_count }}</span>
                    </div>
                </div>
            </div>
        @empty
