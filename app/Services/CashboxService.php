...
class CashboxService
{
     ...
     * Only 'transfer' distributions count towards the recipient's business balance.
     *
     * @param CashPeriod $period
    ...
    public function getBalance(CashPeriod $period, $user): array
    {
        ...
        // Calculate received from distributions - ONLY 'transfer' type counts as business money
        ...
        $receivedFromTransfers = CashTransaction::where('cash_period_id', $period->id)
            ...
            ->where('distribution_type', CashTransaction::DISTRIBUTION_TYPE_TRANSFER)
            ->sum('amount');

        $received = $receivedFromDeposits + $receivedFromRefunds + $receivedFromTransfers;
    ...
    public function getOrCreateCurrentPeriod(int $companyId): CashPeriod
    {
        ...
        return CashPeriod::firstOrCreate(
            [
